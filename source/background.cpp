...
struct VAO *create_Background_vao(const GLenum primitive_mode, const int numVertices,
                              const GLfloat *vertex_buffer_data,
                              GLenum fill_mode, const vector<tuple<int, int, int>> &triangle_groups);

Background_Obj::Background_Obj(const float &start_square_center_x, const float &start_square_center_y)
{
    int i, j, k, t, tmp;
    ...
    this->position = glm::vec3(0.0f, 0.0f, middle_z_c);

    this->x_length_rect = start_square_center_x/2.0f;
    this->y_length_rect =start_square_center_y/2.0f;
    ...
    */
    for (i = 0; i < tot_nodes; i++)
    {
        ...
        grid_points[num_features_per_coordinate * i + 5] = -10.0f;
    }
    ...
    */
    ...
    this->object = create_Background_vao(GL_TRIANGLES, tot_nodes, grid_points, GL_FILL, triangle_groups);
    return;
}

void Background_Obj::set_position(const float x, const float y)
{
    this->position = glm::vec3(x, y, 0);
}

void Background_Obj::draw(glm::mat4 VP, const GLuint program_Background_ID)
{
    ...
    setMat4("MVP", MVP, program_Background_ID);

    // void setMat4(const std::string &name, const glm::mat4 &smat, int prog_id)

    GLCall(draw_background(this->object, this->num_elements_in_indices));
}
