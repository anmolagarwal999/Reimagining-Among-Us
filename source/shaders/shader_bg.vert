#version 330 core

// input data : sent from main program
layout (location = 0) in vec3 vertexPosition;
layout (location = 1) in vec3 vertexColor;

uniform mat4 MVP;
uniform vec2 origin_coordinates;

// output data : used by fragment shader
out vec3 fragColor;
out vec4 coeffs;

void main ()
{
    vec4 v = vec4(vertexPosition, 1.0f); // Transform an homogeneous 4D vector

    // The color of each vertex will be interpolated
    // to produce the color of each fragment
    fragColor = vertexColor;
    //fragColor = vec3(vertexPosition);
    float dx=(vertexPosition.x-origin_coordinates.x);
    float dy=(vertexPosition.y-origin_coordinates.y);

    float dis=dx*dx+dy*dy;
    if(dis<3115.0f)
    {
        coeffs=vec4(1.0f, 1.0f, 1.0f, 1.0f);
    }
    else
    {
        coeffs=vec4(0.5f, 0.3f, 0.1f, 1.0f);

    }

    // Output position of the vertex, in clip space : MVP * position
    gl_Position = MVP * v;
}
